...
export function DocsSidebar() {
  ...
  return (
    <aside className="w-64 shrink-0 hidden md:flex flex-col border-r border-border-subtle h-[calc(100vh-64px)] sticky top-16 py-8 pr-6">
      <nav className="flex-1 overflow-y-auto flex flex-col gap-8 pb-8">
        {navItems.map((section) => (
          <div key={section.title}>
            <h4 className="text-xs font-bold text-text-tertiary mb-3 tracking-wider">
              {section.title}
            </h4>
      ...
      {/* Social Links Footer */}
      <div className="mt-auto pt-6 border-t border-border-subtle flex items-center gap-5">
